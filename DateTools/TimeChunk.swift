...
public struct TimeChunk {
  ...
  public func totalSeconds() -> Int {
    ...
    total += self.minutes * Int(Constants.SecondsInMinute)
    total += self.hours * Int(Constants.SecondsInHour)
    total += self.days * Int(Constants.SecondsInDay)
    total += self.months * Int(Constants.SecondsInMonth30)
    total += self.weeks * Int(Constants.SecondsInWeek)
    total += self.years * Int(Constants.SecondsInYear)
    return total
  }
  ...
  func to(_ unit: TimeUnits) -> Int {
    ...
    if (unit == .seconds) {
      ...
      return totalSeconds() / Int(Constants.SecondsInMinute)
    } else if (unit == .hours) {
      return totalSeconds() / Int(Constants.SecondsInHour)
    } else if (unit == .days) {
      return totalSeconds() / Int(Constants.SecondsInDay)
    } else if (unit == .weeks) {
      return totalSeconds() / Int(Constants.SecondsInWeek)
    } else if (unit == .years) {
      return totalSeconds() / Int(Constants.SecondsInYear)
    }
    return 0
  }
  
  func toDecimal(_ unit: TimeUnits) -> Double {
    if Double(self.months) != Double(0) {
      print("Months are not supported for conversion due to their uncertain number of days.")
      return 0.0
    }
    
    let totalSeconds = Double(self.totalSeconds())
    if (unit == .seconds) {
      return Double(totalSeconds)
    } else if (unit == .minutes) {
      return Double(totalSeconds) / Double(Constants.SecondsInMinute)
    } else if (unit == .hours) {
      return Double(totalSeconds) / Double(Constants.SecondsInHour)
    } else if (unit == .days) {
      return Double(totalSeconds) / Double(Constants.SecondsInDay)
    } else if (unit == .weeks) {
      return Double(totalSeconds) / Double(Constants.SecondsInWeek)
    } else if (unit == .years) {
      return Double(totalSeconds) / Double(Constants.SecondsInYear)
    }
    return 0.0
  }
